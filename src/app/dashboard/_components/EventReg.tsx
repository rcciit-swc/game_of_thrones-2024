  ...
  event: string;
    ...
    useEffect(() => {
        const fetchEventsData = async () => {
          const events = await fetchEvents(event);
  ...
  return (
    <>
      <div
        ...
        } inset-0 bg-opacity-25  overflow-y-auto overflow-x-hidden   z-50 justify-center items-center w-full md:inset-0  max-h-full`}
      >
        <div className="relative p-4 w-full max-w-2xl h-screen my-auto flex items-center mx-auto justify-center ">
          <Modal
            show={openModal}
            onClose={() => setOpenModal(false)}
            className=" pt-[10vh]"
          >
            <Modal.Header className="bg-body shadow-sm shadow-white  rounded-t-xl ">
              Event Registration
            </Modal.Header>
            <Modal.Body className="bg-body ">
              <form
                className="flex flex-col overflow-y-scroll items-start gap-5 overflow-x-hidden rounded-md bg-[#252525] px-10 py-5 shadow-sm shadow-white"
